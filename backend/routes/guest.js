...
router.put('/cart/:id/modifiers', requireGuestToken, (req, res) => {
  try {
    ...
    const item = db.prepare('SELECT * FROM guest_cart_items WHERE id = ? AND guest_token = ?').get(Number(req.params.id), req.guestToken);
    ...
    console.error('Guest modifier update error:', err);
